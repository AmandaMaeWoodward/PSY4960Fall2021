...
```{r}
ggplot(sleepstudy, aes(Days, Reaction)) + geom_point()+ facet_wrap(~Subject)+ geom_smooth(method="lm")
```

4. Run a linear mixed model with a random intercept. 
```{r}
mm1<- lmer(Reaction ~ Days + (1|Subject), data=sleepstudy)
summary(mm1)
```
...
```{r}
mm2<- lmer(Reaction~Days +(Days|Subject), data=sleepstudy)
summary(mm2)
```
